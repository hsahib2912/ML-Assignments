...
class LogRegression(object):
    ...
    def fit(self,train_x,train_y,test_x,test_y):
        ...
        for i in range(self.max_iter):
            ...
            dcoeff = (1/train_x.shape[1]) * (np.dot(train_x.T,y_hat-train_y) + self.lam*np.sum(self.coeff))
...
def perform_grid_search(train_s,train_l,test_s,test_l):
    lam_list = [0.01,0.05,.1,1,2,3,5,10,20,50]
    acc = []
    for lam in  lam_list:
...
train_s,train_l,test_s,test_l = nFold(samples,labels,5,4)               #Grid search for lambda value
...
for i in range(5):
    ...
    plt.savefig('Graphs/Q2_d_Acc_fold_'+str(i)+'.png')
    #plt.show()
    plt.close()
    ...
    plt.savefig('Graphs/Q2_d_Loss_fold_'+str(i)+'.png')
    #plt.show()
    plt.close()
